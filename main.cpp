...
int main(int argc, char **argv) {
	...
	if(argc != 11){
		printf("Usage: give M, N, F, NNZ, NNZ_TEST, lambda, mu, X_BATCH, THETA_BATCH and DATA_DIR.\n");
		printf("E.g., for netflix data set, use: \n");
		printf("./main 17770 480189 100 99072112 1408395 0.048 0 1 3 ./data/netflix/ \n");
		printf("E.g., for movielens 10M data set, use: \n");
		printf("./main 71567 65133 100 9000048 1000006 0.05 0 1 1 ./data/ml10M/ \n");
		printf("E.g., for yahooMusic data set, use: \n");
		printf("./main 1000990 624961 100 252800275 4003960 1.4 0 6 3 ./data/yahoo/ \n");
		return 0;
	}
	...
	int X_BATCH = atoi(argv[8]);
	int THETA_BATCH = atoi(argv[9]);
	std::string DATA_DIR(argv[10]);
	printf("M = %d, N = %d, F = %d, NNZ = %ld, NNZ_TEST = %ld, lambda = %f, mu = %f\nX_BATCH = %d, THETA_BATCH = %d\nDATA_DIR = %s \n",
			m, n, f, nnz, nnz_test, lambda, mu, X_BATCH, THETA_BATCH, DATA_DIR.c_str());
	...
	*/
	...
	fread(XTHost_tmp, sizeof(XTHost_tmp[0]), m*f, xinitfile);
	fread(thetaTHost_tmp, sizeof(thetaTHost_tmp[0]), n*f, thetainitfile);
	fclose(xinitfile);
	...
	cudacall(cudaMallocHost( (void** ) &cooValHostTestPtr_tmp, nnz_test * sizeof(cooValHostTestPtr_tmp[0])) );
    ...
    for (int i = 0; i < nnz_test; i++) cooValHostTestPtr[i] = cooValHostTestPtr_tmp[i];
